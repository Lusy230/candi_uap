import 'dart:async';
import 'package:candi_uap/animations/animation_chain.dart';
import 'package:candi_uap/animations/animation_combo_collapse.dart';
import 'package:candi_uap/animations/animation_combo_three.dart';
import 'package:candi_uap/animations/animation_swap_tiles.dart';
import 'package:candi_uap/bloc/bloc_provider.dart';
import 'package:candi_uap/bloc/game_bloc.dart';
import 'package:candi_uap/widget/board.dart';
import 'package:candi_uap/widget/game_moves_left_panel.dart';
import 'package:candi_uap/widget/game_over_splash.dart';
import 'package:candi_uap/widget/game_splash.dart';
import 'package:candi_uap/widget/objective_panel.dart';
import 'package:candi_uap/widget/shadowed_text.dart';
import 'package:candi_uap/helpers/animations_resolver.dart';
import 'package:candi_uap/helpers/array_2d.dart';
import 'package:candi_uap/helpers/audio.dart';
import 'package:candi_uap/models/animation_sequence.dart';
import 'package:candi_uap/models/combo.dart';
import 'package:candi_uap/models/level.dart';
import 'package:candi_uap/models/row_col.dart';
...
class GamePage extends StatefulWidget {
  static Route<dynamic> route(Level level) {
    return MaterialPageRoute(
        builder: (BuildContext context) => GamePage(level: level, key: UniqueKey(),));
  }
...
class _GamePageState extends State<GamePage>
  ...
  late OverlayEntry _overlayEntryFromTile;
  late OverlayEntry _overlayEntryAnimateSwapTiles;
  ...
  Widget _buildBoard() {
    return Align(
      ...
      child: Board(
        ...
        level: widget.level, key: UniqueKey(),
      ),
    );
  }
  ...
  void _onPanDown(DragDownDetails details) {
    ...
    // Reset
    gestureFromTile = Offset as Tile;
    gestureStarted = false;
    gestureOffsetStart = Offset as Offset;
    gestureFromRowCol = Offset as RowCol;
  ...
  void _onPanEnd(_) {
    ...
    gestureOffsetStart = Offset as Offset;
    _overlayEntryFromTile.remove();
    _overlayEntryFromTile = Offset as OverlayEntry;
  }
      ...
      if (test == true) {
        ...
        if (rowCol.col < 0 ||
          ...
          if (canBePlayed) {
            ...
            _overlayEntryFromTile.remove();
            _overlayEntryFromTile = OverlayEntry as OverlayEntry;
                ...
                builder: (BuildContext context) {
                  return AnimationSwapTiles(
                    ...
                    onComplete: () async {
                      ...
                      _overlayEntryAnimateSwapTiles.remove();
                      _overlayEntryAnimateSwapTiles = OverlayEntry as OverlayEntry;
                        ...
                        if (isSourceTileABomb) {
                          gameBloc.gameController.proceedWithExplosion(
                              Tile(
                                  row: destTile.row,
                                  col: destTile.row,
                                  type: gestureFromTile.type, level: Level),
                              gameBloc);
                        }
                ...
                onComplete: () {
                  ...
                  if (pendingSequences == 0) {
                    ...
                    overlayEntries.forEach((OverlayEntry entry) {
                      entry.remove();
                      entry = OverlayEntry as OverlayEntry;
                    });
    ...
    _gameSplash = OverlayEntry(
        ...
        builder: (BuildContext context) {
          return GameOverSplash(
            ...
            onComplete: () {
              _gameSplash.remove();
              _gameSplash = OverlayEntry as OverlayEntry;
